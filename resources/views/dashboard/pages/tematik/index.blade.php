                    ...
                    <div class="card-header pb-0">
                        <div class="row mb-3">
                            ...
                            <div class="col-lg-3 text-end mt-1">
                                <a href="{{ route('dashboard.tematik.create') }}" class="btn btn-primary btn-sm d-inline">Tambah
                                    {{ $page }}</a>
                            </div>
                        </div>
                    </div>
                                        ...
                                        @foreach ($datas as $row)
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex px-3 py-1">
                                                        <div class="d-flex flex-column justify-content-center">
                                                            <h6 class="mb-0 text-sm">{{ $row->tematik }}</h6>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td class="align-middle text-center text-sm">
                                                    <a href="{{ route('dashboard.tematik.edit', $row->id) }}"
                                                        class="badge bg-primary"><i class="bi bi-pencil-fill"></i></a>
                                                    {{-- <form id="deleteForm" action="{{ route('dashboard.tematik.destroy', $row->id) }}"
                                                    ...
                                                    <a href="#" onclick="deleteTematik('{{ route('dashboard.tematik.destroy', $row->id) }}')"><span
                                                        class="badge badge-sm bg-gradient-warning"><i
